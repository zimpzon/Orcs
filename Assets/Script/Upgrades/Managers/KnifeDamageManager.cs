...
namespace Assets.Script.Upgrades
{
    public static class KnifeDamageManager
    {
        public static string GetText()
        {
            ...
            sb.AppendLine("");
            sb.AppendLine($"<color=#dddddd>Total damage: <color=COLOR-ARENA>{Format256.Format(SaveGame.Members.TotalDamageDaggerThrow)}</color>");
        ...
        {
            Decimal256 priceForNext = UpgradeProgression.PriceX2(UpgradeProgression.InitialPrice_DaggerDamage_X2, SaveGame.Members.LevelKnifeDamageX2 + 1);
            if (priceForNext > SaveGame.Members.Money)
                return;
